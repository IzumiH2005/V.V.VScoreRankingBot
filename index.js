...
const parser = {
    parseQuizText(text) {
        ...
        for (const line of lines) {
            ...
            // Format amélioré pour capturer les noms avec différents préfixes
            let participantMatch = null;
            
            // Pattern général qui capture: [emoji/symboles optionnels] [- optionnel] [nom] [points]
            // Supporte: 🛡 - nom points, ⚔️ - nom points, - nom points, @nom points, nom points
            ...
            if (participantMatch) {
                ...
                // Nettoyer le nom en supprimant les caractères spéciaux en début/fin
                ...
                if (rawName && rawName.length >= 2 && !isNaN(points) && points > 0 && !isRubriqueMatch) {
                    // Conserver le nom nettoyé
...
const commands = {
    handleQuizMessage(msg) {
        ...
        try {
            ...
            // Aucune rubrique détectée, utiliser l'ancien système (50 points)
            quizData.modePoints = 50;
            this.processQuizUpdate(msg.chat.id, quizData, quizId, date);
    ...
    processQuizUpdate(chatId, quizData, quizId, date) {
        ...
        // Envoyer le rapport
        bot.sendMessage(chatId, report).catch(err => {
            console.error('Erreur envoi rapport:', err);
        });
        
        // Envoyer le classement (avec gestion automatique des messages longs)
        if (leaderboardText.length > 4000) {
            // Créer un fichier temporaire pour le classement
            const filename = `classement_${moment().format('YYYYMMDD_HHmmss')}.txt`;
            ...
            try {
                ...
                bot.sendDocument(chatId, filepath, {
                    caption: '📊 Classement mis à jour\n🏆 Le fichier contient tous les joueurs et leurs nouvelles positions'
                }).then(() => {
                    fs.unlinkSync(filepath);
                }).catch((err) => {
                    console.error('Erreur envoi fichier classement:', err);
                    if (fs.existsSync(filepath)) {
                        fs.unlinkSync(filepath);
                    }
                });
            } catch (err) {
                console.error('Erreur création fichier classement:', err);
            }
        } else {
            bot.sendMessage(chatId, leaderboardText, { parse_mode: 'Markdown' }).catch(err => {
                console.error('Erreur envoi classement:', err);
            });
        }
    },
    ...
    handleMergeCommand(msg, match) {
        ...
        // Parsing amélioré pour gérer les noms composés
        const input = match[1].trim();
        
        // Méthode intelligente de séparation des noms
        // D'abord essayer de séparer par virgules, puis par espaces multiples
        let playerNames = [];
        ...
        if (input.includes(',')) {
            ...
            // Essayer de détecter les noms composés vs noms séparés
            const words = input.split(/\s+/);
            
            // Si on a peu de mots, probablement des noms simples
            if (words.length <= 3) {
                playerNames = words.map(name => name.trim().toUpperCase()).filter(name => name);
            } else {
                // Logique intelligente pour détecter les noms composés
                // Rechercher des correspondances exactes dans le classement d'abord
                const allPlayers = storage.leaderboard.players.map(p => p.name);
                const potentialNames = [];
                
                // Essayer toutes les combinaisons possibles de mots consécutifs
                for (let i = 0; i < words.length; i++) {
                    for (let j = i + 1; j <= words.length; j++) {
                        const candidateName = words.slice(i, j).join(' ').toUpperCase();
                        
                        // Vérifier si ce candidat existe dans le classement
                ...
                // Trier par longueur décroissante pour prioriser les noms composés
                potentialNames.sort((a, b) => b.length - a.length);
                
                // Sélectionner les noms sans chevauchement
                const selectedNames = [];
                ...
                // Si on n'a pas trouvé assez de noms composés, traiter les mots restants individuellement
                for (let i = 0; i < words.length; i++) {
                ...
                // Si la détection intelligente n'a pas fonctionné, revenir à la méthode simple
        ...
        if (playerNames.length < 2) {
            return bot.sendMessage(msg.chat.id, "❌ Usage: /merge [nom1] [nom2] [nom3] ...\n" +
                "Exemples:\n" +
                "• /merge RYOMEN, RYOMEN SK\n" +
                "• /merge PLAYER1 PLAYER2 PLAYER3\n" +
                "• /merge \"LONG NAME\" \"OTHER NAME\"\n" +
        ...
        if (missingPlayers.length > 0) {
            return bot.sendMessage(msg.chat.id, `❌ Joueurs introuvables: ${missingPlayers.join(', ')}\n\n` +
                ...
                `📋 Utilisez la commande exactement comme ils apparaissent dans le classement.`);
        }
...
bot.onText(/\/classement/, (msg) => {
    ...
    // Si le message est trop long pour Telegram (limite ~4096 caractères), l'envoyer comme fichier
    if (leaderboardText.length > 4000) {
        ...
        try {
            ...
            bot.sendDocument(msg.chat.id, filepath, {
                caption: '📊 Classement complet du groupe V.V.V\n🏆 Le fichier contient tous les joueurs et leurs positions'
            }).then(() => {
                // Supprimer le fichier après envoi
                fs.unlinkSync(filepath);
            }).catch((err) => {
                console.error('Erreur envoi fichier classement:', err);
                // En cas d'erreur, diviser le message en chunks
                const chunks = [];
                let currentChunk = "";
            ...
            console.error('Erreur création fichier classement:', err);
            bot.sendMessage(msg.chat.id, "❌ Erreur lors de la génération du classement");
        }
    } else {
        // Message assez court, l'envoyer normalement
